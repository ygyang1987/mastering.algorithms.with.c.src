...
*****************************************************************************/
	...
	fprintf(stdout, "destmp: %02x %02x %02x %02x %02x %02x %02x %02x\n",
		destmp[0], destmp[1], destmp[2], destmp[3], destmp[4],
		destmp[5], destmp[6], destmp[7]);
	...
	fprintf(stdout, "desctx: %02x %02x %02x %02x %02x %02x %02x %02x\n",
		desctx[0], desctx[1], desctx[2], desctx[3], desctx[4],
		desctx[5], desctx[6], desctx[7]);
	...
	fprintf(stdout, "desptx: %02x %02x %02x %02x %02x %02x %02x %02x\n",
		desptx[0], desptx[1], desptx[2], desptx[3], desptx[4],
		desptx[5], desptx[6], desptx[7]);
...
*****************************************************************************/
	...
	for (i = 0; i < 128; i++) {
		...
		if (rsatmp == rsaptx)
			fprintf(stdout,
				"rsatmp=%5d, rsactx=%5d, rsaptx=%5d (OK)\n",
				rsatmp, rsactx, rsaptx);
		else
			fprintf(stdout,
				"rsatmp=%5d, rsactx=%5d, rsaptx=%5d (ERROR)\n",
				rsatmp, rsactx, rsaptx);
