...
*****************************************************************************/
	...
	for (i = 0; i < MSTVCT; i++) {
		...
		if ((mst_vertex =
		     (MstVertex *) malloc(sizeof(MstVertex))) == NULL)
			return 1;
	...
	for (element = list_head(&span); element != NULL; element =
		...
		fprintf(stdout, "vertex=%s, parent=%s\n",
			...
			mst_vertex->parent !=
			NULL ? (char *)mst_vertex->parent->data : "*");
...
*****************************************************************************/
	...
	for (i = 0; i < PTHVCT; i++) {
		...
		if ((pth_vertex =
		     (PathVertex *) malloc(sizeof(PathVertex))) == NULL)
			return 1;
	...
	for (element = list_head(&paths); element != NULL; element =
		...
		fprintf(stdout, "vertex=%s, parent=%s, d=%.1lf\n",
			...
			pth_vertex->parent !=
			NULL ? (char *)pth_vertex->parent->data : "*",
			pth_vertex->d);
...
*****************************************************************************/
	...
	for (i = 0; i < TSPVCT; i++) {
		...
		if ((tsp_vertex =
		     (TspVertex *) malloc(sizeof(TspVertex))) == NULL)
			return 1;
		...
		if (list_ins_next(&vertices, list_tail(&vertices), tsp_vertex)
		    != 0)
			return 1;
