		    ...
		    **original)
{
	...
	unsigned char window[LZ77_WINDOW_SIZE],
	    buffer[LZ77_BUFFER_SIZE], *orig, *temp, next;
   ...
   **************************************************************************/
      ...
      ***********************************************************************/
			...
			for (i = 0; i < LZ77_WINOFF_BITS; i++) {
				...
				bit_set((unsigned char *)&offset, tpos,
					bit_get(compressed, ipos));
				ipos++;
			...
			for (i = 0; i < LZ77_BUFLEN_BITS; i++) {
				...
				bit_set((unsigned char *)&length, tpos,
					bit_get(compressed, ipos));
				ipos++;
			...
			for (i = 0; i < LZ77_NEXT_BITS; i++) {
				...
				tpos =
				    ...
				    LZ77_NEXT_BITS + i;
				bit_set((unsigned char *)&next, tpos,
					bit_get(compressed, ipos));
				ipos++;
